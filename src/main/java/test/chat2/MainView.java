...
import test.chat2.service.Storage;
...
public class MainView extends VerticalLayout implements AppShellConfigurator {
    ...
    private void buildChat() {
        ...
        grid.addColumn(new ComponentRenderer<>(message -> new Html(renderRow(message))))
                .setAutoWidth(true);
        grid.addColumn(new ComponentRenderer<>(message -> new Html(renderUsers(storage.getUsers()))))
                .setWidth("400px").setHeader("Online users");
        TextField field = new TextField();
    ...
    private String renderUsers(Set<String> users) {
        return Processor.process(String.format("**%s**", users));
    }
